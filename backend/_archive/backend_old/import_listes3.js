...
db.exec(`CREATE TABLE IF NOT EXISTS distributions (
  ...
  code_article TEXT NOT NULL, nom TEXT,
  store_id TEXT NOT NULL, date_envoi TEXT NOT NULL,
  liste TEXT, saison TEXT
)`);
...
db.transaction(() => {
  for (const file of files) {
    const wb = XLSX.readFile(path.join('../data/listes/', file));
    const rows = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1 });
    const headers = rows[1];

    // Detecter index Reference automatiquement (contient 'f' et 'rence' ou position 7/8)
    let idxRef = headers.findIndex(h => String(h).toLowerCase().includes('f') && String(h).toLowerCase().includes('rence'));
    if (idxRef === -1) idxRef = headers.findIndex(h => String(h).toLowerCase().includes('ref'));
    ...
    console.log(`${file}: ref=${idxRef} date=${idxDate}`);

    let count = 0;
    for (let i = 2; i < rows.length; i++) {
      const row = rows[i];
      const code = String(row[idxRef] || '').trim();
...
console.log('Articles distincts:', db.prepare('SELECT COUNT(DISTINCT code_article) as n FROM distributions').get().n);
console.log('Dates min/max:', db.prepare('SELECT MIN(date_envoi) as min, MAX(date_envoi) as max FROM distributions').get());
